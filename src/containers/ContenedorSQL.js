import { generateString } from './StringGenerator.js.js';
...
export default class ContenedorSQL {
	...
	async save(item) {
		const timestamp = ContenedorSQL.timestamp();
		const fullItem = [...item, timestamp];
		const id = await this.database(this.table).insert(fullItem, ['id']);
	}
